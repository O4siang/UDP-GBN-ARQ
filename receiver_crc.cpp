...
void GBNUDP_receive() {
	...
	while(closeFlag) {
        ...
        //Doing the CRC32 checksum, simulate there is 20% probabilty that the received data being modified
    	//If checksum isn't matched, discard the packet
        int status;
        	...
        	if (packet.type == '2' && packet.seq == 0) {
        		...
        		if (packet.seq == waitSeq) {
                    ...
                    if (crc == packet.crc) {
                        ...
                        printf("packet discarded due to corruption\n");
                        continue;
                    }
