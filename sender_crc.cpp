...
void GBNUDP_send(unsigned char* data);
struct dataPacket createSynPkt(int seq_no);
struct dataPacket createDataPkt(int seq_no, unsigned char* data);
...
void GBNUDP_send(unsigned char* data) {
  ...
  int lastSeq = strlen((char*)data); //count input length
